  ...
  bool appl::processCommand(std::ostream& os) {
    ...
    if (pos1 == CONST_CMD_HELP) {
      ...
      if (pos2.length()){
		...
		comps_t::const_iterator deps_it = deps.begin();
		comps_t::const_iterator deps_eit = deps.end();
        for (; deps_it != deps_eit; ++deps_it){
          os << (*deps_it)->getId() << std::endl;
          // build inherits list
		  comps_t inhs;
          (*deps_it)->buildInheritsFromList(inhs);
		  comps_t::const_iterator inhs_it = inhs.begin();
		  comps_t::const_iterator inhs_eit = inhs.end();
          os << " *";
          for (; inhs_it != inhs_eit; ++inhs_it){
            os << (*inhs_it)->getId() << " ";
          }
          os << std::endl;
          // compose context
